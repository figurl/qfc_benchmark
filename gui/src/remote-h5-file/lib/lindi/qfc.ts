...
export const zstdDecompress = async (buf: ArrayBuffer): Promise<ArrayBuffer> => {
  return new Promise((resolve) => {
    ZstdCodec.run((zstd: any) => {
      const simple = new zstd.Simple();
      const decompressed = simple.decompress(new Uint8Array(buf));
...
const qfc_multi_segment_decompress = async (o: {
  ...
  if (compression_method === "zlib") {
    ...
    decompressedArray = new Int16Array(await zstdDecompress(buf));
  } else {
...
export const _get_segment_ranges = (
  total_length: number,
  segment_length: number,
): [number, number][] => {
...
export const concatenateInt16Arrays = (arrays: Int16Array[]): ArrayBuffer => {
...
export const concatenateFloat32Arrays = (arrays: Float32Array[]): ArrayBuffer => {
  const total_length = arrays.reduce((acc, x) => acc + x.length, 0);
...
const isQtcCompressionOpts = (x: any): x is qtcCompressionOpts => {
  if (!x) return false;
  if (typeof x !== "object") return false;
  if (x.compression_method !== "zlib" && x.compression_method !== "zstd")
    return false;
  if (x.dtype !== "float32" && x.dtype !== "int16") return false;
  if (x.id !== "qfc_qtc") return false;
  if (typeof x.quant_scale_factor !== "number") return false;
  if (typeof x.zlib_level !== "number") return false;
  if (typeof x.zstd_level !== "number") return false;
  return true;
...
const qtc_decompress = async (o: {
  ...
  const { buf, dtype, quant_scale_factor, compression_method } = o;
  ...
  if (dtype === "int16") {
    const ret = new Int16Array(decompressedArray.length);
    for (let i = 0; i < decompressedArray.length; i++) {
      ret[i] = Math.round(decompressedArray[i] / quant_scale_factor);
    }
    return ret;
  }
  else {
    throw Error(`dtype not implemented: ${dtype}`);
  }
}

export const qtcDecompress = async (
  buf: ArrayBuffer,
  shape: number[],
  compressor: qtcCompressionOpts,
): Promise<any> => {
  if (!isQtcCompressionOpts(compressor)) {
    console.warn(compressor);
    throw Error("Invalid qtc compressor");
  }
  ...
  const decompressed_buf = await qtc_decompress({
    buf: buf.slice(4 * 4),
    dtype: compressor.dtype,
    num_channels,
    num_samples,
