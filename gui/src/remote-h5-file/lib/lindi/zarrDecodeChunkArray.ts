...
import { qfcDecompress, qtcDecompress } from "./qfc";
import { qwcDecompress } from "./qwc";
...
const zarrDecodeChunkArray = async (
  ...
  if (compressor) {
    ...
    } else if (compressor.id === "qfc_qtc") {
      if (!shape) {
        throw Error("No shape for qtc");
      }
      ret = await qtcDecompress(chunk, shape, compressor);
    } else if (compressor.id === "qfc_qwc") {
      if (!shape) {
        throw Error("No shape for qwc");
      }
      ret = await qwcDecompress(chunk, shape, compressor);
    } else {
